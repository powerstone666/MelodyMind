...
function Artist({ names }) {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        if (res.data?.data?.results) {          setMusicInfo(
  ...
  const ArtistCard = ({ image, name, onClick }) => {
    return (      <div
        onClick={onClick}
        className="bg-melody-purple-800 rounded-lg overflow-hidden transition-all duration-300 hover:scale-102 cursor-pointer flex flex-col items-center p-4 border border-melody-purple-700 hover:border-melody-pink-500/50 min-w-[130px] max-w-[130px] md:min-w-[160px] md:max-w-[160px] lg:min-w-[180px] lg:max-w-[180px] xl:min-w-[190px] xl:max-w-[190px]"
      >
        <div className={`relative overflow-hidden rounded-full mb-3 ${isAboveMedium ? 'h-28 w-28' : 'h-20 w-20'}`}>
          {image ? (
            <img
              src={image}
              alt={name}
            ...
            <div className="h-full w-full flex items-center justify-center bg-gray-700 text-gray-400">
              <svg xmlns="http://www.w3.org/2000/svg" className={`${isAboveMedium ? 'h-10 w-10' : 'h-8 w-8'}`} fill="none" viewBox="0 0 24 24" stroke="currentColor">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1} d="M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z" />
              </svg>
            </div>
          )}
        </div>
        <h3 className={`font-medium text-center truncate max-w-full ${isAboveMedium ? 'text-sm' : 'text-xs'}`}>
          {name}
        </h3>
      </div>
    );
    ...
    >
      {loading ? (
        ...
        <>          {(isAboveMedium ? musicInfo.slice(0, limit) : musicInfo).map((artist) => (
            <Link key={artist.id} to="/innerartist" className="block">
              <ArtistCard
                image={artist.image}
                name={artist.name}
